...
class Phen2GeneExecTests(unittest.TestCase):
    ...
    def test_calc_empty(self):
        res = self.phen2gene.execute([''])
        assert res == {'query': {}, 'response': {}}
        res = self.phen2gene.execute(['', ''])
        assert res == {'query': {}, 'response': {}}
        HPOS = 'ABC:00000001, HP:0099999, HP_0030422'
        res = self.phen2gene.execute(HPOS.split(','), rows=10)
        assert file_equal(FOLDER, res, F'empty.json')

    def test_calc_mixed(self):
        HPOS = 'ABC:00000001, HP:0099999, HP_0030422, HP_0030152, HP_0045013, HP:0012758'
        res = self.phen2gene.execute(HPOS.split(','), rows=10)
        assert file_equal(FOLDER, res, F'mixed.json')
