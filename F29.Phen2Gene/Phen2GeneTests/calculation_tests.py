...
class CalculationTests(unittest.TestCase):
    ...
    def test_calc_empty(self):
        res = calculate(KBASE_PATH, [''])
        assert res == {}
        res = calculate(KBASE_PATH, ['', ''])
        assert res == {}

    def test_calc_unknown(self):
        res = calculate(KBASE_PATH, ['ABC:00000001'])
        assert res == {}
        res = calculate(KBASE_PATH, ['HP:0099999', 'HP:99999999'])
        assert res == {}
        ...
        res = calculate(KBASE_PATH, HPOS.split(','), normalize=False, rows=20)
        assert file_equal(FOLDER, res, F'dravet.json')
        res = calculate(KBASE_PATH, HPOS.split(','), normalize=True, rows=20)
